#!/usr/bin/env python3
"""Compare pool kernel behavior_test timing against the most-recent run.

Reads the freshly-written pooling_test_report.json, compares per-test
duration_ns against pool_timing_last.json (the previous run's snapshot),
prints a comparison table, then overwrites pool_timing_last.json with
...
DEFAULT_REPORT = "build/kernels/pool/kv260/pooling_test_report.json"
DEFAULT_BASELINE = "build/kernels/pool/kv260/pool_timing_last.json"
...
def main() -> int:
    ...
    ap.add_argument("--report",   default=DEFAULT_REPORT,
                    help=f"path to pooling_test_report.json (default: {DEFAULT_REPORT})")
    ...
    if not os.path.exists(args.report):
        ...
        print("       run `make behavior_test_pool` first.", file=sys.stderr)
        return 2
