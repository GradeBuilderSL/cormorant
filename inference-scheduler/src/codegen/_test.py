...
class _TestMixin:
    ...
    def generate_test(self) -> str:
        ...
        return (
            ...
            "\n"
            "/*\n"
            " * Default hardware instance name passed to inference_init().\n"
            " *\n"
            " * On Linux the XVectoropkernel driver scans /sys/class/uio/uioN/name\n"
            " * and opens the first UIO device whose sysfs name matches this string.\n"
            " * It is NOT a /dev/uioN path -- it is the name assigned in the Vivado\n"
            " * block design (check: cat /sys/class/uio/uio0/name on the board).\n"
            " *\n"
            " * On bare-metal the name must match the IP instance name in\n"
            " * xparameters.h (e.g. XPAR_VECTOROPKERNEL_0_DEVICE_ID).\n"
            " *\n"
            " * Override at build time: cmake -DINFERENCE_TEST_INSTANCE=my_name\n"
            " *          or at runtime: ./test_inference my_name\n"
            " */\n"
            "#ifndef INFERENCE_TEST_INSTANCE\n"
            "#  ifdef __linux__\n"
            "#    define INFERENCE_TEST_INSTANCE  \"fabric\"\n"
            "#  else\n"
